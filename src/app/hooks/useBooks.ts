  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        switch (type) {
          case 'search':
            url = `https://openlibrary.org/search.json?q=${encodeURIComponent(query || '')}&fields=*`;
            const searchResponse = await fetch(url);
            const searchData = await searchResponse.json();
            setBooks(searchData.docs.filter((book: any) => book.cover_edition_key));
            break;
          case 'book':
          ...
          case 'genre':
            url = `https://openlibrary.org/subjects/${encodeURIComponent(query || '')}.json?details=true`;
